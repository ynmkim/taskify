...
>(({ className, ...props }, ref) => (
  <AvatarPrimitive.Image ref={ref} className={cn('aspect-square h-full w-full', className)} width="100%" {...props} />
));
...
>(({ className, children, ...props }, ref) => (
  <AvatarPrimitive.Fallback
    ref={ref}
    className={cn('flex h-full w-full items-center justify-center rounded-full bg-green-a3c4a2', className)}
    {...props}
  >
    <span style={{ color: '#fff', fontSize: '16px', fontWeight: 600 }}>{children}</span>
  </AvatarPrimitive.Fallback>
));
