...
const TodoCardDialog = ({card, columnTitle, onClick}:{card:Card, columnTitle:string, onClick:(cardId:number) => void}) => {
  ...
  return(
    <Dialog open={open}>
      ...
      <DialogOverlay onClick={handleOpenChange} className="bg-black-000000/40 w-screen h-screen fixed top-0 left-0 z-[5]"/>
      <DialogContent className="fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] z-10">
        <TodoCardModal card={card} columnTitle={columnTitle} onClick={handleDeleteButton} toggleModal={handleOpenChange}/>
      </DialogContent>
    </Dialog>
  )
};
